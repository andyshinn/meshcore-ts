...
// ProtocolError(undefined) — the documented bare-RESP_ERR semantics — rather
// than reading past the frame. setDeviceTime awaits the RESP_OK/RESP_ERR ack
// channel and re-throws ProtocolError, so it surfaces the parsed errorCode.
describe('RESP_ERR frame-length handling on the ack path', () => {
  let stop: (() => void) | undefined;
  afterEach(() => stop?.());

  it('a bare (1-byte) RESP_ERR rejects with ProtocolError and errorCode undefined', async () => {
